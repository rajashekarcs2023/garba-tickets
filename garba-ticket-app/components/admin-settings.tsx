...
import { logout, saveSettingsAction } from "@/app/actions"
    ...
    <main className="min-h-screen bg-muted/30">
      <div className="mx-auto max-w-3xl p-4 md:p-8">
        <header className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-rose-700">Garba — Admin settings</h1>
            <p className="text-sm text-muted-foreground">
              Signed in as <span className="font-medium text-foreground">{admin}</span>. Changes reflect in the agent
              within ~1 minute.
            </p>
          </div>
          <form action={logout}>
        ...
        {!configReady ? (
          <Card>
            ...
            <CardContent className="text-sm text-muted-foreground">
              ...
              <code>NOTION_CONFIG_DATABASE_ID</code> (and <code>NOTION_TOKEN</code>). This same id goes on the agent so
              it reads your changes live.
            </CardContent>
          </Card>
        ) : (
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {form.price_tiers.length === 0 ? (
                  <p className="text-sm text-muted-foreground">No tiers — every ticket is {form.currency_symbol}
                    {form.ticket_price_usd}.</p>
                ) : (
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                {form.points_of_contact.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    No POCs yet — the agent will fall back to its env default.
                  </p>
                ) : (
